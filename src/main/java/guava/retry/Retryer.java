...
public final class Retryer<V> {
   ...
   * @param rejectionPredicate the predicate used to decide if the attempt must be rejected or not.
   *                           If an attempt is rejected, the retryer will retry the call, unless
   *                           the stop strategy indicates otherwise or the thread is interrupted.
   */
  ...
  /**
   * Executes the given callable. If the rejection predicate accepts the attempt, the stop strategy
   * is used to decide if a new attempt must be made. Then the wait strategy is used to decide how
   * much time to sleep and a new attempt is made.
   *
   * @param callable the callable task to be executed
   * @return the computed result of the given callable
   * @throws ExecutionException if the given callable throws an exception, and the rejection
   *                            predicate considers the attempt as successful. The original
   *                            exception is wrapped into an ExecutionException.
   * @throws RetryException     if all the attempts failed before the stop strategy decided to
   *                            abort, or the thread was interrupted. Note that if the thread is
   *                            interrupted, this exception is thrown and the thread's interrupt
   *                            status is set.
   */
  public V call(Callable<V> callable) throws ExecutionException, RetryException {
    ...
    for (int attemptNumber = 1; ; attemptNumber++) {
      ...
      try {
        ...
        attempt = new ResultAttempt<V>(result, attemptNumber,
            TimeUnit.NANOSECONDS.toMillis(System.nanoTime() - startTime));
      } catch (Throwable t) {
        attempt = new ExceptionAttempt<V>(t, attemptNumber,
            TimeUnit.NANOSECONDS.toMillis(System.nanoTime() - startTime));
      }
  ...
  /**
   * A {@link Callable} which wraps another {@link Callable} in order to add retrying behavior from
   * a given {@link Retryer} instance.
   *
   * @author JB
   */
