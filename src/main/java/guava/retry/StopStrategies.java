...
public final class StopStrategies {
   ...
   * services with this kind of behavior when small wait intervals between retry attempts are being
   * used.
   *
   ...
   * this {@link StopStrategy} will check if the amount of time that's passed from the first attempt
   * has exceeded the given delay amount. If it has exceeded this delay, then using this strategy
   * causes the retrying to stop.
   *
  ...
  private static final class StopAfterAttemptStrategy implements StopStrategy {
    ...
    public StopAfterAttemptStrategy(int maxAttemptNumber) {
      Preconditions.checkArgument(maxAttemptNumber >= 1, "maxAttemptNumber must be >= 1 but is %d",
          maxAttemptNumber);
